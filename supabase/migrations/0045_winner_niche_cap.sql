-- ════════════════════════════════════════════════════════════════════════
-- 0045_winner_niche_cap — różnorodność branż w topie winnerów (Część A).
-- p_niche_cap (domyślnie 3) = max produktów z jednej niche w tiered top (proven+fresh).
-- Pool cienki po capie → mniej (nie dopychamy). Tail (p_tiered=false) bez capu.
-- ════════════════════════════════════════════════════════════════════════
drop function if exists public.product_winners(int, text, boolean, boolean);
create or replace function public.product_winners(
  p_limit int default 10, p_country text default null, p_tiered boolean default true, p_dedup_window boolean default false, p_niche_cap int default 3
)
returns jsonb language sql stable set search_path = public as $$
  ...
  ranked as (
    select *,
      ...
      row_number() over (partition by niche order by score desc) as niche_rank
    from e
  )
  select coalesce(jsonb_agg(j order by blk, ord), '[]'::jsonb)
  from (
    select j, 1 as blk, proven_rank as ord from ranked where p_tiered and is_proven_b and proven_rank <= (p_limit * 7 / 10) and niche_rank <= p_niche_cap
    union all
    select j, 2 as blk, fresh_rank as ord from ranked where p_tiered and is_fresh_b and fresh_rank <= (p_limit - p_limit * 7 / 10) and niche_rank <= p_niche_cap
    union all
    select j, 0 as blk, score_rank as ord from ranked where (not p_tiered) and score_rank <= p_limit
  ) z;
$$;
grant execute on function public.product_winners(int, text, boolean, boolean, int) to anon, authenticated, service_role;
