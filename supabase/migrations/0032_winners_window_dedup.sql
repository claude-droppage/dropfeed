-- ════════════════════════════════════════════════════════════════════════
-- 0032_winners_window_dedup — bez powtórek w oknie 7 dni (Część 3).
-- p_dedup_window=true (dzienny snapshot + feed): wyklucza produkty I marki obecne
-- w snapshotach poprzednich 6 dni + jeden-na-markę dziś. Po wyłączeniu <10 → mniej
-- (NIE dopychamy). Gdy dzień wypada z okna, produkt/marka może wrócić (re-odkrycie).
-- p_dedup_window=false → ogon/browse bez tej dedup (pełna lista). Reszta jak 0029.
-- ════════════════════════════════════════════════════════════════════════
...
returns jsonb language sql stable set search_path = public as $$
  ...
  e as (
    select d.*, jsonb_build_object(
      'product_id', id, 'name', name, 'brand_name', brand_name, 'logo_url', logo_url, 'avatar_initials', avatar_initials,
...
-- snapshot dnia: z dedup oknem (bez powtórek z ostatnich 6 dni) + jeden-na-markę
...
begin
  ...
  from jsonb_array_elements(public.product_winners(p_limit, null, true, true)) with ordinality as t(w, ord);
