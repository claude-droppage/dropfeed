-- ════════════════════════════════════════════════════════════════════════
-- 0034_winner_page_id — Część 4: page_id (fb_page_id marki) w product_winners
-- i product_detail → link 'reklamy' prowadzi na STRONĘ MARKI w FB Ad Library
-- (view_all_page_id, country=ALL), nie keyword-search. Reszta product_winners jak 0032.
-- ════════════════════════════════════════════════════════════════════════
...
returns jsonb language sql stable set search_path = public as $$
  select jsonb_build_object(
    ...
    'offer_url', p.offer_url, 'brand_name', b.name, 'store_url', b.store_url, 'page_id', b.fb_page_id,
    ...
    'ads', (
      select coalesce(jsonb_agg(jsonb_build_object('id', d.id, 'thumb_url', d.thumb_url, 'creative_url', d.creative_url, 'heat', d.heat, 'format', d.format) order by d.heat desc), '[]'::jsonb)
      from (select distinct on (a.creative_url) a.id, a.creative_url, coalesce(a.thumb_url, a.creative_url) as thumb_url, a.heat_score as heat, a.format
            from ads a where a.product_id = p.id and a.is_active order by a.creative_url, a.heat_score desc) d
    )
  )
  from products p left join brands b on b.id = p.brand_id where p.id::text = p_id;
$$;
