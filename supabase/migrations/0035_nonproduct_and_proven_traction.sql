-- ════════════════════════════════════════════════════════════════════════
-- 0035_nonproduct_and_proven_traction — Część A (filtr nie-produktów) + B (proven z trakcją).
-- A: is_nonproduct(name, offer_url) — blocklista nazw (B2B/eventy/apki/platformy, EN+PL) +
--    host offer_url (app store / alibaba / aliexpress / dhgate / temu). Konfigurowalna.
--    BEZPIECZNIK: tylko oczywiste; null offer_url NIE flagujemy (nie ubijamy realnych).
-- B: proven wymaga WŁASNEJ trakcji produktu — ad_count(produkt) ≥2 LUB stores_count ≥2.
--    Marka-gigant z 1 reklamą produktu (460-dniowy starzec) wypada z proven.
-- ════════════════════════════════════════════════════════════════════════
...
$BODY$;
grant execute on function public.is_nonproduct(text, text) to anon, authenticated, service_role;
