-- ════════════════════════════════════════════════════════════════════════
-- 0039_eligible_and_feed_shuffle — JEDNA wspólna definicja „eligible" (winnerzy + swipe feed)
-- + feed_shuffle (swipe feed = ten sam pool, BEZ scoringu, czyste losowanie po seedzie).
-- platform: confirmed non_shopify OUT; unknown/null ZOSTAJE (bezpiecznik). Ingest dalej all-tagged.
-- ════════════════════════════════════════════════════════════════════════

create or replace function public.is_eligible_product(p_name text, p_brand text, p_category text, p_offer_url text, p_platform text)
...
$BODY$;
grant execute on function public.is_eligible_product(text, text, text, text, text) to anon, authenticated, service_role;
...
returns jsonb language sql stable set search_path = public as $$
  ...
  floored as (select * from agg where oldest_age >= 7 and public.is_eligible_product(name, brand_name, category, offer_url, platform)),
  classified as (
    select *,
...
-- Swipe feed v2: ten sam pool eligible, BEZ scoringu — czyste losowanie md5(ad_id||seed).
-- Seed per wejście/refresh (klient), stały w sesji → paginacja bez duplikatów, inna kolejność/seed.
