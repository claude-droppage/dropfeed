-- ════════════════════════════════════════════════════════════════════════
-- 0029_brand_active_total — realny sygnał „proven" z nagłówka strony marki.
-- Zabija biweekly reconcile: zamiast LICZYĆ aktywne reklamy marki przez pełny
-- re-scrape, ODCZYTUJEMY pole `total` (nagłówek „X active ads" per reklamodawca)
-- z lekkiego requestu strony marki (limitPerSource=1), tylko dla KANDYDATÓW, cache 7 dni.
-- product_winners: active_brand_ads = brand_active_total.fb_total (świeże ≤14 dni) →
-- fallback brand_daily_snapshot → licznik stored ads. Reszta (tiering 70/30) bez zmian.
-- + purge efemerycznych winnerów >8 dni (KROK 3).
-- ════════════════════════════════════════════════════════════════════════

create table if not exists public.brand_active_total (
  brand_id   uuid primary key references public.brands (id) on delete cascade,
  fb_total   int not null,
  page_id    text,
  fetched_at timestamptz not null default now()
);
alter table public.brand_active_total enable row level security;
drop policy if exists bat_select on public.brand_active_total;
create policy bat_select on public.brand_active_total for select using (true);
...
returns jsonb language sql stable set search_path = public as $$
  ...
  -- „marka zarabia" = realny nagłówek strony marki (brand_active_total), świeży ≤14 dni
  ...
  agg as (
    ...
    group by p.id, p.name, p.niche, p.price_in_store, p.offer_url, b.name, b.store_url, b.logo_url,
             b.avatar_initials, s.stores_count, m.momentum_delta, br.fb_total, bs.snap_ads, sa.c
  ),
  ...
  classified as (
    select *,
      ...
      ( active_brand_ads + new_ads_7d * 4 + greatest(0, momentum_delta) * 3
        + least(stores_count, 10) * 4 + oldest_age * 0.3 ) as score
    from floored
  ),
  deduped as (
  ...
  e as (
    select d.*,
      jsonb_build_object(
        ...
        'active_brand_ads', active_brand_ads, 'rep_video', rep_video, 'rep_thumb', rep_thumb,
        'is_video', (rep_video is not null), 'is_fresh', is_fresh_b,
        'is_validated', (stores_count >= 3), 'is_scaling', (momentum_delta > 0 or new_ads_7d > 0),
...
-- KROK 3: winners efemeryczne — przy każdym snapshocie czyść starsze niż 8 dni.
