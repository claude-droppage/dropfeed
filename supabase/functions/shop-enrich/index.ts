...
Deno.serve(async (req) => {
  ...
  // (2) powiązane wideo (v0, smart query) — best-effort
  if (!p.videos_fetched_at || now - Date.parse(p.videos_fetched_at) > STALE_MS) {
    ...
    const vids = await apifyRunSync('clockworks~tiktok-scraper', { searchQueries: [q], resultsPerPage: 6, searchSection: '/video' })
    if (Array.isArray(vids)) {
      const rows = vids.filter((v) => v?.id).map((v) => ({
        product_id: productId, video_id: String(v.id), url: (v.webVideoUrl as string) ?? null,
        cover_url: ((v.videoMeta as Record<string, unknown>)?.coverUrl as string) ?? null,
        caption: String(v.text ?? '').slice(0, 300), author: ((v.authorMeta as Record<string, unknown>)?.name as string) ?? null,
      ...
      if (rows.length) await supabase.from('tiktok_shop_video').upsert(rows)
