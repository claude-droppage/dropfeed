...
**Etap 1 — w toku (pipeline danych).** Kroki 1–3 gotowe; pozostały kroki 4–6 (enrichment, hosting wideo, automatyzacja). Decyzje orkiestracji zapisane: **model push** (Apify Schedule → webhook → Edge Function), wideo na R2 **bez transkodowania na start**, **pierwszy run wąski** przez `scrape_config`.
