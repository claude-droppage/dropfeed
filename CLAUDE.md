...
**Etap 1 — w toku (pipeline danych).** Kroki 1–2 gotowe; pozostały kroki 3–6 (scraping+ingest, enrichment, hosting wideo, automatyzacja). Decyzje orkiestracji zapisane: **model push** (Apify Schedule → webhook → Edge Function), wideo na R2 **bez transkodowania na start**, **pierwszy run wąski** przez `scrape_config`.
...
- **Pozostałe kroki Etapu 1** (szczegóły zadania: sekcja "Drugie zadanie" niżej):
  - **(3) Apify scraping + ingest** — aktor `curious_coder/facebook-ads-library-scraper` (backup `apify/facebook-ads-scraper`); model push: Apify Schedule → webhook → Supabase Edge Function pobiera dataset z Apify API i robi `upsert` do `raw_ads` (dedup po `ad_archive_id`); pierwszy run wąsko przez `scrape_config` (1-2 nisze, ograniczone kraje, ~500 reklam).
  - **(4) Enrichment Haiku** — batch Claude Haiku na wierszach `raw_ads` z `processed = false`: klasyfikacja `offer_type`/`niche`/`angle`/`hook` + landing fetch → `confidence` (nazwa oferty ukrywana w UI gdy `< 0.7`) + heat score → `ads`, `processed = true`.
  - **(5) Hosting wideo R2** — pobranie kreacji z Meta na Cloudflare R2 **bez transkodowania na start** (`creative_url`/`thumb_url`); kompresja ~3-5 MB = późniejsza optymalizacja (worker `ffmpeg`, NIE Cloudflare Stream).
  - **(6) Automatyzacja** — Apify Schedule (np. codziennie 04:00) + webhook do Edge Function; rozszerzenie zakresu w `scrape_config` po walidacji wąskiego runu.
...
- **Dalsze kroki Etapu 1** (scraping Apify model push, enrichment Haiku, R2 — wideo bez transkodowania na start, automatyzacja) — kroki 1–2 (schemat + warstwa danych) są już gotowe, patrz "Aktualny etap".
