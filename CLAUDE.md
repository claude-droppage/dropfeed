...
**Gate Shopify przy ingescie reklam.** Do bazy `ads` wpadają tylko reklamy prowadzące do Shopify (strona produktowa = bonus, nie wymóg). `lib/shopify.ts`: warstwa URL ($0: `*.myshopify.com`/`/products/`/`/collections/`) + fetch custom domen (sygnatury `cdn.shopify.com`/`/cdn/shop/`/`Shopify.shop`/`x-shopify-*`/„powered by Shopify"). `link_url` z payloadu = finalny URL (NIE redirect FB). `enrich.ts` taguje `ads.platform` (shopify/non_shopify/unknown) + `landing_type` (product/collection/home/other) **reużywając istniejącego fetcha landingu (zero kosztu)**. **v2: NIE dropuje przy ingescie** — wszystko wpada tagowane, filtr „eligible" jest na warstwie zapytań (odwracalne, bez utraty danych; toggle hard-drop opisany w kodzie). **JEDNA wspólna definicja `is_eligible_product(name,brand,category,offer_url,platform)`** (not excluded_title + not nonproduct + not blocked_brand + not beauty_category + `platform<>non_shopify`; unknown/null zostaje) używana przez **`product_winners`** (floored) ORAZ **swipe feed**. `has_product_page` = bonus/badge 🛍.

**Swipe feed (/feed Reklamy) v2** = ten sam pool `is_eligible_product` co winnerzy (tylko Shopify, bez beauty/blocklisty/non-produktów), ale **BEZ scoringu — czyste losowanie**: RPC `feed_shuffle(offset,limit,seed,offer_types,min_age)` `ORDER BY md5(ad_id||seed)`. Seed per wejście/refresh (FeedView, stały w sesji) → paginacja bez duplikatów, inna kolejność/seed/user. Porzucone heat/nisze/jitter/diversity (`feed_page` nieużywane). Pula ~256 ads. Backfill istniejących: `npm run shopify:tag` (URL-layer + fetch, dedup po URL). Rozkład startowy (per URL): ~186 shopify / 206 non_shopify / 109 stron produktowych.

**Blocklista marek kosmetycznych (po POLU MARKI, nie tytule).** `is_blocked_brand(brand)` (migr. 0036, źródło prawdy listy = ta funkcja + mirror tu) — match po znormalizowanej PEŁNEJ nazwie marki (`norm_brand`: lowercase, bez diakrytyków, bez znaków spec.; NIE substring). Seed (rozszerzalny): PL Eveline/Your Kaya/Nutridome/Bielenda/Ziaja/Tołpa/Dr Irena Eris/Lirene/Soraya/Floslek/Semilac/Nacomi/Pharmaceris/Iwostin/Oceanic AA/Bandi; global Vichy/La Roche-Posay/CeraVe/Nivea/Garnier/L'Oréal/Maybelline/Neutrogena/Cetaphil/Eucerin/Bioderma/Avène/The Ordinary/Paula's Choice/COSRX/Beauty of Joseon/Anua/Some By Mi/Medicube/Mixsoon/Innisfree/Laneige/Cocosolis/Dr.Melaxin/Sacheu/Rare Beauty/e.l.f./NYX/Fenty; retail Notino/Sephora/Douglas/Hebe/Rossmann. **Filtr kosmetyków oparty na NICHE (strukturalny, language-agnostic).** `is_eligible_product` wyklucza `niche='beauty'` (enum z enrichmentu — łapie `pielęgnacja i makijaż`, `Sonnenschutz` itd. niezależnie od języka) **POZA narzędziami** (`is_beauty_tool`: gua sha/szczotki/rollery/brush/cupping/massage/device po nazwie LUB kategorii). Wycina 171 konsumpcyjnych, zostawia 62 beauty-tools. `is_beauty_category` = backup (multilingual: skincare/cosmetic/kosmetyk/kosmetik/pielęgnac/schönheit/soin/belleza, bez „tool"/„device"). `is_excluded_title` += DE/FR/ES (sonnenschutz/selbstbräuner/crème solaire/autobronzant/protector solar). `is_blocked_brand` po POLU marki (~53 marki kosmetyczne, +Christian Laurent). Stosowane na wszystkich powierzchniach (winnerzy/feed/TikTok).
...
- **Zakładka „Produkty"** = kalendarz Minea (7 dni, kafelek = miniatura top-zwycięzcy + etykieta; okno [dziś+6 wstecz] w **UTC** — fix dryfu strefy; domyślnie najnowszy dzień Z DANYMI, nigdy pusty; puste dni wyszarzone, `winner_days_rich`) + grid zwycięzców dnia + pełna lista rise-first (live do 60). **Bez powtórek w oknie 7 dni:** `product_winners(p_dedup_window=true)` (snapshot dnia + Reklamy·PL) wyklucza produkty I marki z ostatnich 6 dni + jeden-na-markę; <10 → mniej (nie dopychamy); wraca po wypadnięciu z okna. Wykluczenia + self-tan/SPF/bronzer/sunscreen/sunblock/samoopalacz/przeciwsłoneczn/after-sun. **Bogate karty `WinnerCard`**: wideo reklamy z R2 (hover-play, fallback miniatura), logo fanpage (fallback inicjały), reason chips (⌖ sklepów / ▲ +reklam/7d / ✦ nowy / 🌍), liczba aktywnych reklam, „chodzi N dni", link do sklepu (offer_url), **„reklamy marki" → strona marki FB Ad Library (`view_all_page_id`, country=ALL; brak page_id → „szukaj reklam" keyword, uczciwie)**. BEZ Ad analysis. Kalendarz: DZIŚ = realna data serwera (UTC), domyślnie zaznaczony nawet gdy pusty („zbieram dziś / snapshot w toku"), ZERO dni z przyszłości, etykiety skrót+numer (PT 19) na każdym kaflu.
