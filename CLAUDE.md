...
- **C7 (boardy na koncie) — ODŁOŻONE** do osobnej mini-fazy (boardy nadal w localStorage `dropfeed_boards_v1`). **Domena GOTOWA: `swipespy.io`** wpięta do Vercela; Supabase `site_url=https://swipespy.io` + `uri_allow_list` (localhost + swipespy.io + dropfeed-phi). **Zostało przed launchem:** `mailer_autoconfirm=false` (teraz `true` = dev). Marka = SwipeSpy; infra (repo, vercel `-phi` URL, Supabase `project_id`, klucze `dropfeed_*`) celowo zostaje „dropfeed".
