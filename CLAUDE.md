...
Etap 1 ukończony (pipeline autonomiczny + wydajność + FAZA A/B + wdrożenie) — patrz "Aktualny etap". Otwarte kierunki: **logika feedu** (PRD §11: miękkie ważenie nisz + wstrzykiwanie różnorodności — teraz sort heat-only z limitem 10/markę); **Etap 3** (auth: Supabase Auth Google + magic link, boardy/preferencje per user, migracja z localStorage, limit swipe'ów); **Etap 4** (monetyzacja: freemium, Stripe, paywall); skalowanie scrape (tiered) przy większej liczbie userów.
