...
**Gate Shopify przy ingescie reklam.** Do bazy `ads` wpadają tylko reklamy prowadzące do Shopify (strona produktowa = bonus, nie wymóg). `lib/shopify.ts`: warstwa URL ($0: `*.myshopify.com`/`/products/`/`/collections/`) + fetch custom domen (sygnatury `cdn.shopify.com`/`/cdn/shop/`/`Shopify.shop`/`x-shopify-*`/„powered by Shopify"). `link_url` z payloadu = finalny URL (NIE redirect FB). `enrich.ts` taguje `ads.platform` (shopify/non_shopify/unknown) + `landing_type` (product/collection/home/other) **reużywając istniejącego fetcha landingu (zero kosztu)** i **bramkuje**: confirmed `non_shopify` NIE wpada; `unknown` wpada+flaga (toggle `GATE_DROP_UNKNOWN`). `product_winners` wyklucza `non_shopify` (unknown/null zostaje) + bonus/badge `has_product_page` (🛍). Backfill istniejących: `npm run shopify:tag` (URL-layer + fetch, dedup po URL). Rozkład startowy (per URL): ~186 shopify / 206 non_shopify / 109 stron produktowych.
