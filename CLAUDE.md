...
**Etap 1 — w toku (pipeline danych).** Kroki 1–4 gotowe; pozostały kroki 5–6 (hosting wideo, automatyzacja). Decyzje orkiestracji zapisane: **model push** (Apify Schedule → webhook → Edge Function), wideo na R2 **bez transkodowania na start**, **pierwszy run wąski** przez `scrape_config`. **Feed pokazuje realne dane** (492 reklamy); mock usunięty z bazy po kroku 4 (kod seeda został jako fallback dev).
