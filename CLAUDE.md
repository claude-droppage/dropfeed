...
- **Następne kroki Etapu 1:** Apify (scraping Meta Ad Library → `raw_ads`, **model push**: Apify Schedule → webhook → Edge Function) → enrichment Claude Haiku (klasyfikacja `offer_type`/`niche`/`angle`/`hook` + `confidence` + heat score → `ads`) → Cloudflare R2 (pobieranie wideo **bez transkodowania na start**, `creative_url`/`thumb_url`; kompresja = późniejsza optymalizacja). Pierwszy run wąsko przez `scrape_config`. Szczegóły zadania: sekcja "Drugie zadanie" niżej.
